...
def aplicar_jitter(latencia, jitter):
    subprocess.run(f"tc qdisc add dev eth0 root netem delay {latencia}ms {jitter}ms",
        shell=True,
        check=True)
...
def main(): 
    ...
    archivo = "Mediciones_Linux_tls_1_3.csv"
    ...
    limpiar_red()
    print("\n Datos guardados en Mediciones_Linux_tls_1_3.csv")
