...
def medir_tls():
    """
    Mide tiempos de conexion TCP, handshake TLS 1.2 y total con curl.
    """
    cmd = (
        f'{CURL} --connect-timeout 5 '
        f'--tlsv1.2 --tls-max 1.2 '
...
def main():
    muestras = 50
    archivo = "dataset_tls_1_2_mac.csv"
    ...
    with open(archivo, mode='w', newline='') as f:
        ...
        for lat in niveles_latencia:
            ...
            for i in range(muestras):
                datos = medir_tls()
                fila = ["macOS", "Parametro", "Latencia", lat, 0, 0, "tls12", i + 1] + datos
        ...
        for p in niveles_perdida:
            ...
            for i in range(muestras):
                datos = medir_tls()
                fila = ["macOS", "Parametro", "Perdida", p, 0, p, "tls12", i + 1] + datos
                writer.writerow(fila)
                print(f"  Muestra {i+1}/{muestras} -> handshake={datos[2]} ms")
        ...
        for jitter in niveles_jitter:
            ...
            for i in range(muestras):
                datos = medir_tls()
                fila = ["macOS", "Parametro", "Jitter", jitter, jitter, 0, "tls12", i + 1] + datos
                writer.writerow(fila)
                print(f"  Muestra {i+1}/{muestras} -> handshake={datos[2]} ms")
        ...
        for nombre, lat, jitter, perdida in escenarios:
            ...
            for i in range(muestras):
                datos = medir_tls()
                fila = ["macOS", "Escenario", nombre, lat, jitter, perdida, "tls12", i + 1] + datos
                writer.writerow(fila)
                print(f"  Muestra {i+1}/{muestras} -> handshake={datos[2]} ms")
